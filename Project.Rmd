...
We just assume there are not many of them, and it's not worth writing longer code
...
from the higher weights of body parts
...
the two groups differ significantly in diameter. 
Mollusks with 15 rings have a higher diameter
...
```
It probably shows a non-linear relation between them.
...
```
It's R^2 is 0.857, and the residual errors we get are a bit too high, though. 
...
then their volume is proportional to the surface area.
The square should be dependent on diameter: S = π*d^2 : 4
This means that S (and total weight) should have a linear relation with d^2
...
However, the intercept is way below zero, and this model is poor at predicting the
weight at low diameters. The possible explanation here is that this sample forms a part of a logistic curve:
As we can see from the last plot, it has a plateau at low diameters, with a certain lowest critical weight
(beyond which they can't go, packing their organs tighter instead). In the same way,
...
Also, it would be interesting to see how we can predict age by other parameters.
Or, oppositely, predict any changes to be associated with age?
It looks to be so for height and diameter at least.

removing the couple of outliers for scaling:
```{r}
...
it "seems" that they stop growing when the mollusk gets 10-11 rings.
They vary highly, however, and it is hard to call these results significant.
...
meaning that their number corresponds with age well.
...
and they mostly go to the "juvenile" group at low ring numbers.
## R Markdown
